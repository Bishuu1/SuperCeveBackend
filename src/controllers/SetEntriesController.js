...
SetEntriesCtrl.getSetEntries = async (req, res) => {
  await ConjuntoSchema.find(
    { Usuario: req.query.Usuario },
    (err, Conjuntos) => {
      //CONSULTA QUE BUSCA LOS CONJUNTO DE ENTRADAS
...
SetEntriesCtrl.getSetEntry = async (req, res) => {
  ...
  const Conjunto = await ConjuntoSchema.findById(
    ConjuntoId,
    (err, Conjunto) => {
      //CONSULTA QUE BUSCA LA ENTRADA POR ID
    ...
    EntradaSchema.find(
      ...
      function (err, Entradas) {
        return res.status(200).send({
          Nombre: Conjunto.NombreConjuntoEntradas,
...
SetEntriesCtrl.updateSetEntries = async (req, res) => {
  ...
  await ConjuntoSchema.findByIdAndUpdate(
    ConjuntoId,
    Update,
    (err, ConjuntoUpdated) => {
      if (err) {
...
SetEntriesCtrl.getEntries = async (req, res) => {
  await EntradaSchema.find({ Usuario: req.query.Usuario }, (err, Entradas) => {
    //CONSULTA QUE BUSCA LAS ENTRADAS
    if (err) {
      return res.status(500).send.length({ message: 'error' });
    }
    if (!Entradas) {
      return res.status(404).send({ message: 'Error al buscar' });
    } else {
...
SetEntriesCtrl.getEntry = async (req, res) => {
  ...
  const Entrada = await EntradaSchema.findById(EntradaId, (err, Entrada) => {
    //CONSULTA QUE BUSCA LA ENTRADA POR ID
    if (err) {
...
SetEntriesCtrl.updateEntries = async (req, res) => {
  const EntradaId = req.params.EntradaId;
  const Update = req.body;
  await EntradaSchema.findByIdAndUpdate(
    EntradaId,
    Update,
    (err, EntradaUpdated) => {
      if (err) {
